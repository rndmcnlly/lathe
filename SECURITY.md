...
- **Secret handling** — the `env_vars` UserValve gives credentials to every model-controlled shell command, including delegate commands. These values are masked in the OWUI settings UI but are not hidden from the model, which can read its command environment. Treat them as credentials entrusted to the model and use narrowly scoped, revocable tokens. A bash wrapper contains shell-quoted plaintext values only until execution starts: it self-unlinks before exporting them. Completed-command sidecars are scrubbed once Lathe observes completion; a short lease prevents another OWUI worker from deleting a session before its background poller collects the final log. Normal logs are then removed; a truncated foreground command's log remains available until the next `bash()` call so the documented spill path can be read. Sessions supporting live background processes retain only PID markers and are reaped by a later `bash()` call after those processes exit. Sandbox stop or restart clears all `/dev/shm` sidecars and in-memory Daytona sessions. Cross-user disclosure or retention beyond these stated lifetimes would be a critical issue.
